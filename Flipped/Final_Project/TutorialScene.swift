...
class TutorialScene: SKScene, SKPhysicsContactDelegate {
    ...
    private let background = SKSpriteNode(imageNamed: "white2.jpg")
    ...
    override func didMove(to view: SKView) {
        ...
        NotificationCenter.default.addObserver(self, selector: #selector(TutorialScene.rotated), name: NSNotification.Name.UIDeviceOrientationDidChange, object: nil)
    ...
    func updateCamera(){
        ...
        if portrait == true {
            jumpButton?.position = CGPoint(x: character.position.x, y: character.position.y - 280)
            pauseButton?.position = CGPoint(x: character.position.x, y: character.position.y + 280)
        }
        if portrait == false {
            jumpButton?.position = CGPoint(x: character.position.x, y: character.position.y - 250)
            pauseButton?.position = CGPoint(x: character.position.x, y: character.position.y + 280)
        }
